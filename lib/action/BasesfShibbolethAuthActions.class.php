 ...
 * this module's 'login' action URL (via a <Location> block).
 *
 ...
 *
 */
class BasesfShibbolethAuthActions extends sfActions
{
  /**
  ...
  {
    $result = $this->enforceLoginOnHostname();
    if ($result)
    {
      return $result;
    }
  ...
  private function enforceLoginOnHostname()
  {
    $hostname = sfConfig::get('app_sfShibboleth_login_on_hostname', false);
    if ($hostname)
    {
      $request = $this->getRequest();
      $hostnow = $request->getHost();
      if ($hostnow !== $hostname)
      {
        $uri = $request->getUri();
        $uri = preg_replace('/' . preg_quote($hostnow, '/') . '/', $hostname, $uri, 1);
        // Do both in one step to avoid chicken and egg problems
        // with different canonicalization rules for http and https
        if (sfConfig::get('app_sfShibboleth_secure_after_login', false))
        {
  ...
  /**
   * Capture the intended URL and redirect to the actual
   * Shibboleth-protected login action
   */
  public function executePreLogin(sfWebRequest $request)
  {
    if ($this->getUser()->isAuthenticated()) 
    {
      // This can happen if the user goes directly to /login.
      // Don't redirect them in an infinite loop when they
      // are already logged in
      return $this->redirect('@homepage');
    }
    $uri = $request->getUri();
    if (sfConfig::get('app_sfShibboleth_secure_after_login'))
    {
      $uri = preg_replace('/^http:/', 'https:', $uri);
    }
    $this->getUser()->setAttribute('sfShibboleth_after', $uri);
